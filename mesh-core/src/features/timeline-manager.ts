...
export class TimelineManager {
  ...
  private timelines: Map<string, TimelineMetadata> = new Map();
   ...
   */
  async createTimeline(input: TimelineCreateInput): Promise<TimelineInfo> {
   ...
   */
  async applyPatches(input: TimelinePatchInput): Promise<void> {
   ...
   */
  async runInTimeline(input: TimelineRunInput): Promise<TimelineRunResult> {
    const metadata = this.timelines.get(input.timelineId);
    if (!metadata) {
      throw new Error(`Timeline ${input.timelineId} not found`);
    }
   ...
   */
  async compareTimeline(timelineId: string): Promise<TimelineCompareResult> {
    ...
    try {
      ...
      const { stdout: diffStat } = await execAsync(
        `cd "${metadata.workTreePath}" && git diff --stat ${metadata.baseBranch}`,
      ...
      const { stdout: diffNames } = await execAsync(
        `cd "${metadata.workTreePath}" && git diff --name-status ${metadata.baseBranch}`,
        { maxBuffer: 10 * 1024 * 1024 }
      );
      ...
      const fileChanges = diffNames
        ...
        .map(line => {
          ...
          return {
            path: filePath,
            status: (status === "A" ? "added" : status === "D" ? "deleted" : "modified") as "added" | "modified" | "deleted",
   ...
   */
  async promoteTimeline(timelineId: string): Promise<void> {
    const metadata = this.timelines.get(timelineId);
    if (!metadata) {
      throw new Error(`Timeline ${timelineId} not found`);
    }
   ...
   * Clean up a timeline
   */
  async abandonTimeline(timelineId: string): Promise<void> {
    const metadata = this.timelines.get(timelineId);
    if (!metadata) {
      throw new Error(`Timeline ${timelineId} not found`);
    }
    ...
    for (const file of files) {
      if (file.endsWith(".json")) {
        try {
          ...
          const metadata: TimelineMetadata = JSON.parse(content);
  ...
  private metadataToInfo(metadata: TimelineMetadata): TimelineInfo {
    return {
      id: metadata.id,
      name: metadata.name,
      baseBranch: metadata.baseBranch,
